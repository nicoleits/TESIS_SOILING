...
def grafico_barras_mbe(tabla, out_path):
    ...
    metodos = tabla["metodo_evaluado"].tolist()
    bar_colors = [color_metodo(m) for m in metodos]
    ax.bar(x, tabla["MBE_pp"], color=bar_colors, edgecolor="gray")
    ax.axhline(0, color="black", linewidth=0.8, linestyle="-")
    ax.set_xticks(x)
    ax.set_xticklabels(ticklabels_mathtext(metodos), rotation=35, ha="right", fontsize=12)
    ax.set_ylabel("MBE (pp)")
    if FuncFormatter is not None:
        # Eje Y sin decimales (quitar dos ceros)
        ax.yaxis.set_major_formatter(FuncFormatter(lambda x, pos: f"{x:.0f}"))
    ax.set_title(
        f"Sesgo medio respecto a {etiqueta_metodo_mathtext(REFERENCIA)}\n"
        + titulo_reemplazar_iv600_pmax_isc("(SR semanal Q25; IV600 Pmax/Isc valor absoluto)"),
    )
    plt.tight_layout()
...
def grafico_error_vs_semana(df, out_path):
    ...
    for metodo in metodos:
        ...
        ax.plot(d.index, d["error_pp"], "o-", markersize=4,
                label=f"{etiqueta_metodo_mathtext(metodo)} (n={len(d)})",
    ...
    ax.set_title(
        r"Error con signo (SR$_m$ − SR$_\mathrm{ref}$) por semana — ref.: "
        + etiqueta_metodo_mathtext(REFERENCIA),
