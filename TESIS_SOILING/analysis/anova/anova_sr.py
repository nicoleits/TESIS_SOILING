...
Uso (desde TESIS_SOILING):
  ...
  python -m analysis.anova.anova_sr [sr_semanal_largo.csv] [out_dir] [--violin-excluir "PV Glasses"]
"""
import os
import sys
import logging
...
def cargar_datos(norm_largo_csv):
    ...
    if "sr_norm" not in df.columns and "sr_q25" in df.columns:
        # Permite reutilizar el pipeline también con datos SIN normalizar.
        df = df.rename(columns={"sr_q25": "sr_norm"})
...
def grafico_violin(df_pool, df_inter, out_path, excluir_instrumentos=None):
    """
    excluir_instrumentos: nombres exactos de columna instrumento (ej. ["PV Glasses"])
    a omitir solo en este gráfico; no altera ANOVA ni heatmaps.
    """
    if excluir_instrumentos:
        ex = set(excluir_instrumentos)
        df_pool = df_pool[~df_pool["instrumento"].isin(ex)].copy()
        df_inter = df_inter[~df_inter["instrumento"].isin(ex)].copy()
    ...
    for ax, data, title in zip(axes, [df_pool, df_inter], titles):
        instrumentos = orden_instrumentos(data["instrumento"].unique())
        ...
        for i, pc in enumerate(parts["bodies"]):
            pc.set_facecolor(color_metodo(instrumentos[i]))
            pc.set_alpha(0.7)
        ax.set_xticks(range(n))
        ax.set_xticklabels(ticklabels_mathtext(instrumentos), rotation=20, ha="right", fontsize=16)
    ...
    axes[0].set_ylabel("SR (%)", fontsize=17)
    axes[0].tick_params(axis="y", labelleft=True)
    axes[1].tick_params(axis="y", labelleft=False)
    axes[1].set_ylabel("")
    fig.suptitle(titulo_reemplazar_iv600_pmax_isc(
        "Distribución del SR semanal Q25 por instrumento (IV600 Pmax/Isc valor absoluto)"),
...
def _heatmap_pvalores(matrix_df, title, out_path, instrumentos=None):
    ...
    ax.set_xticklabels(ticklabels_mathtext(matrix_df.columns), rotation=35, ha="right", fontsize=12)
    ax.set_yticklabels(ticklabels_mathtext(matrix_df.index), fontsize=12)
...
def generar_reporte(conjuntos, out_path, modo_txt):
    lines = [
        f"# Análisis ANOVA — SR Semanal Q25 ({modo_txt})",
        "",
        f"**Variable dependiente:** SR semanal Q25 ({modo_txt})  ",
...
def run(norm_largo_csv, out_dir, violin_excluir_instrumentos=None):
    os.makedirs(out_dir, exist_ok=True)

    df_all = cargar_datos(norm_largo_csv)
    base_name = os.path.basename(norm_largo_csv).lower()
    ...
    if MATPLOTLIB_AVAILABLE:
        grafico_violin(
            ...
            excluir_instrumentos=violin_excluir_instrumentos,
        )

    generar_reporte(resultados, os.path.join(out_dir, "anova_report.md"), modo_txt)
    return True
...
def main():
    ...
    argv = sys.argv[1:]
    violin_excluir = None
    if "--violin-excluir" in argv:
        i = argv.index("--violin-excluir")
        if i + 1 >= len(argv):
            logger.error("--violin-excluir requiere una lista separada por comas")
            sys.exit(1)
        violin_excluir = [x.strip() for x in argv[i + 1].split(",") if x.strip()]
        argv = argv[:i] + argv[i + 2 :]
    if len(argv) > 0:
        norm_csv = os.path.abspath(argv[0])
    if len(argv) > 1:
        out_dir = os.path.abspath(argv[1])
    logger.info("Input: %s | Salida: %s", norm_csv, out_dir)
    if violin_excluir:
        logger.info("Violin sin instrumentos: %s", violin_excluir)
    ok = run(norm_csv, out_dir, violin_excluir_instrumentos=violin_excluir)
