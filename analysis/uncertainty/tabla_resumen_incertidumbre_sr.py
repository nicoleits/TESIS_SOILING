...
def build_tabla_resumen_incertidumbre_sr(base_dir=None, out_dir=None):
    """
    ...
    csv_path = os.path.join(out_dir, "resumen_incertidumbre_sr_por_metodologia.csv")
    tabla.to_csv(csv_path, index=False)
    logger.info("Tabla resumen incertidumbre SR: %s", csv_path)

    # Markdown
    md_path = os.path.join(out_dir, "resumen_incertidumbre_sr_por_metodologia.md")
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# Resumen comparativo: incertidumbre de SR por metodología\n\n")
