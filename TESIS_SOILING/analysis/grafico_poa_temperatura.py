...
def grafico_poa(project_root, output_path=None):
    ...
    ax.set_ylabel("Irradiancia (W/m²)", fontsize=15)
    ax.set_title("Cálculo de POA (Plane of Array)\nGHI, DHI, DNI (Solys2) y POA en el plano del panel (pvlib)", fontsize=16, pad=12)
...
def grafico_temperatura(project_root, output_path=None):
    ...
    ax.set_title("Temperatura de módulos fotovoltaicos\n(1TE416: módulo sucio, 1TE418: módulo referencia)", fontsize=16, pad=12)
    ax.legend(loc="upper right", fontsize=14)
    ax.tick_params(axis="both", labelsize=13)
    ax.xaxis.set_major_formatter(FuncFormatter(_fmt_month_es))
    ax.xaxis.set_major_locator(mdates.MonthLocator(interval=2))
...
def grafico_poa_con_temperatura(project_root, output_path=None):
    ...
    ax1.set_ylabel("POA (W/m²)", color="k", fontsize=15)
    ...
    ax2.set_ylabel("Temperatura (°C)", color="k", fontsize=15)
    ...
    ax1.legend(lineas_1 + lineas_2, labels_1 + labels_2, loc="lower left", fontsize=13)
