...
def grafico_heatmap_concordancia(mat_r, mat_n, out_path):
    ...
    ax.set_xticklabels(ticklabels_mathtext(mat_r.columns), rotation=40, ha="right", fontsize=12)
...
def grafico_heatmap_ccc(mat_ccc, mat_n, out_path):
    ...
    ax.set_xticks(range(n))
    ax.set_yticks(range(n))
    ax.set_xticklabels(ticklabels_mathtext(mat_ccc.columns), rotation=40, ha="right", fontsize=12)
    ax.set_yticklabels(ticklabels_mathtext(mat_ccc.index), fontsize=12)
    ...
    if _formatter_coma is not None:
        cbar.ax.yaxis.set_major_formatter(FuncFormatter(_formatter_coma))
    ax.set_title(titulo_reemplazar_iv600_pmax_isc(
        "Concordancia intermetodológica — CCC de Lin\nSR semanal Q25 (IV600 Pmax/Isc valor absoluto; resto norm. t₀=100%)"))
    plt.tight_layout()
    plt.savefig(out_path, dpi=220)
    plt.close()
    ...
    for i, inst_y in enumerate(instrumentos):
        for j, inst_x in enumerate(instrumentos):
            ...
            if len(sub) < 2:
                ...
                continue
            ax.scatter(sub[inst_x], sub[inst_y], alpha=0.6, s=25,
                       c=color_metodo(inst_x), edgecolors="none")
            if i == 0:
                ax.set_title(etiqueta_metodo_mathtext(inst_x), fontsize=11)
            if j == 0:
                ax.set_ylabel(etiqueta_metodo_mathtext(inst_y), fontsize=11)
            ...
            ax.text(0.05, 0.95, r_txt, transform=ax.transAxes, fontsize=10, va="top")

    fig.suptitle(titulo_reemplazar_iv600_pmax_isc(
        "Concordancia intermetodológica — Dispersión por pares (IV600 Pmax/Isc valor absoluto)"), fontsize=14)
    plt.tight_layout()
    plt.savefig(out_path, dpi=220)
    plt.close()
...
def run(out_dir=None, norm_csv=None):
    ...
    base_name = os.path.basename(norm_csv or "").lower()
    ...
    with open(readme, "w", encoding="utf-8") as f:
        ...
        if sin_normalizar:
            f.write("Datos: SR semanal Q25 (SIN normalizar); PVStand e IV600 corregidos por T.\n\n")
        else:
        ...
        f.write("```bash\npython -m analysis.concordancia.concordancia_intermetodologica\n```\n\n")
        if sin_normalizar:
            f.write("Requiere tener generado antes `analysis/stats/sr_semanal_q25.csv` (ej. `python -m analysis.stats.agregacion_semanal`).\n")
        else:
