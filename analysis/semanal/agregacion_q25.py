...
def _grafico_series_semanales(df_ancho, out_path):
    ...
    ax.set_ylabel("SR Q25 semanal (%)", fontsize=12)
    ax.set_title("Soiling Ratio — Agregación semanal Q25 por metodología", fontsize=13, pad=10)
...
def _grafico_boxplot_dispersion(df_largo, out_path):
    ...
    ax.axhline(100, color="gray", linestyle="--", linewidth=0.8, alpha=0.6)
    ax.set_ylabel("SR Q25 semanal (%)", fontsize=12)
    ax.set_title("Dispersión del SR semanal (Q25) por metodología", fontsize=13, pad=10)
    ax.set_ylim(bottom=80)
