...
def grafico_sr_por_periodo(df_res, df_resumen, out_path):
    ...
    ax.legend(handles[:3], labels[:3], fontsize=11, loc="lower left")
...
def grafico_sr_por_vidrio(df_res, out_path):
    ...
    handles = [plt.Line2D([0], [0], marker=MARCADORES[m], color="w", markerfacecolor=colors_m[m], markersize=9)
               for m in ["A", "B", "C"]]
    ax.legend(handles, ["Vidrio A", "Vidrio B", "Vidrio C"], fontsize=11, loc="upper right")
    if _formatter_coma is not None:
...
def grafico_curva_acumulacion(df_resumen, out_path):
    ...
    z = np.polyfit(x, y, 1) if len(x) >= 2 else None
    r2_fit = np.nan
    if z is not None and len(x) >= 2:
        y_pred = np.polyval(z, x)
        ss_res = float(np.sum((y - y_pred) ** 2))
        ss_tot = float(np.sum((y - np.mean(y)) ** 2))
        r2_fit = 1.0 - (ss_res / ss_tot) if ss_tot > 0 else np.nan

    # Solo marcadores + barras de error (sin línea entre puntos)
    ax.errorbar(
        df["dias_ref"],
        df["sr_q25"],
        ...
        capthick=1.5,
        elinewidth=1.2,
        label="SR Q25 ± desv. estándar",
    )
    if z is not None and len(x) >= 2:
        pend = z[0]
        sp = f"{pend:.4f}".replace(".", ",")
        sr2 = f"{r2_fit:.3f}".replace(".", ",") if np.isfinite(r2_fit) else "—"
        ...
        ax.plot(
            ...
            linewidth=1.25,
            alpha=0.78,
            label=f"Tendencia lineal: {sp} pp/día, R² = {sr2}",
        )
    ...
    # Ejes más compactos que el formateador global (.3f): X en días enteros, Y en un decimal (%)
    if FuncFormatter is not None:
        ax.xaxis.set_major_formatter(
            FuncFormatter(lambda xv, pos: f"{xv:.0f}".replace(".", ","))
        )
        ax.yaxis.set_major_formatter(
            FuncFormatter(lambda yv, pos: f"{yv:.1f}".replace(".", ","))
        )
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(out_path, dpi=220, bbox_inches="tight")
...
def grafico_datos_detalle_por_vidrio(df_res, n_dias_poa, out_path):
    ...
    for col, muestra in enumerate(vidrios):
        ...
        ax.grid(True, alpha=0.3)
        if col == 2:
            ax.legend(loc="upper right", fontsize=10, bbox_to_anchor=(1.22, 1.0))
...
def main():
    ...
    if len(argv_pos) >= 1:
        out_dir = os.path.abspath(argv_pos[0])
